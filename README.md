...
python backtest_technical.py --market jp --years 2             # 日経225(大型株、実験的)
python backtest_technical.py --market jp-growth --years 2      # 東証グロース250(小型株、実験的)
```
...
高値からの下落率でトレーリングストップ)、`--max-concurrent`(同時保有N銘柄まで、
空きが出るまで新規建てしない)、`--max-atr-pct`(ボラティリティ過大な銘柄を除外)、
`--min-day-score`/`--max-day-score`(その日の上位スコアで絞る)。
これらは`souzai.net`の信用取引戦略記事を参考に追加したが、**検証の結果は
市場によって真逆**だった(米国の小型株には効果ありだが日本にはどちらも逆効果)。

**日本株について**: `--market jp`(日経225、大型株)と`--market jp-growth`
(東証グロース市場250、小型成長株)はいずれもyfinance経由(`<証券コード>.T`)で
テクニカルのみ検証します。財務データ(J-Quants無料プランの制約)を使わないため、
この検証だけは実行できます。日本は**大型株の方が小型株より一貫して良い成績**
(勝率57.4% vs 41.1%)で、米国の「小型株×カタリスト」パターンはそのまま
当てはまりませんでした。詳細と実測値は`notes/DESIGN.md`の8.6〜8.8を参照。
